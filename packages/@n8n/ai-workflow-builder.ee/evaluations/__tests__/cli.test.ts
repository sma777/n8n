...
describe('CLI', () => {
	...
	describe('runV2Evaluation()', () => {
		...
		describe('webhook notification', () => {
			it('should send webhook notification when webhookUrl is provided', async () => {
				...
				expect(mockSendWebhookNotification).toHaveBeenCalledWith(
					expect.objectContaining({
						webhookUrl: 'https://example.com/webhook',
						suite: 'llm-judge',
						experimentName: 'local-evaluation',
			...
			it('should use experiment name from args for langsmith backend', async () => {
				mockParseEvaluationArgs.mockReturnValue(
					createMockArgs({
						webhookUrl: 'https://example.com/webhook',
						backend: 'langsmith',
			...
			it('should use default experiment name for langsmith backend when not specified', async () => {
				mockParseEvaluationArgs.mockReturnValue(
					createMockArgs({
						webhookUrl: 'https://example.com/webhook',
						backend: 'langsmith',
